...
void mitad_num_inv(int *p) {
    *p = *p / 2;
}

void suma_cifras(int *p) {
    int temp = *p;
    int suma = 0;
    while (temp > 0) {
        suma = suma + (temp % 10);
...
void procesar_enigma(int *valor_referencia) {
    invertir_num(valor_referencia);
    mitad_num_inv(valor_referencia);
    suma_cifras(valor_referencia);
}
